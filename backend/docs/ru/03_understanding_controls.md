...
**Что делает:** Удаляет все события темпа из MIDI-файла и заменяет их одним фиксированным значением. Весь файл будет воспроизводиться в одном постоянном BPM.

**Когда использовать:** При объединении MIDI-файлов из разных источников AI-транскрипции (например, Suno + Klang.io) с расходящимися картами темпа. Из-за этого дорожки рассинхронизируются в Guitar Pro, так как каждый источник встроил свою кривую темпа. Принудительный BPM устраняет конфликт.

**Рекомендация:** По умолчанию выключено (OFF). Включайте только при рассинхронизации дорожек из-за конфликтующих карт темпа. Установите значение BPM, соответствующее реальному темпу вашей песни (диапазон 40–300).
